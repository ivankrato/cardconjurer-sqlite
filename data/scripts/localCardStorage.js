// Card storage backed by the server-side SQLite API.
// All card data is persisted via /api/cards  (proxied to the Node.js server).

var localStorageCardKeyList = []
var selectedCardKey = ''

// ── helpers ──────────────────────────────────────────────────────────────────

function rebuildSelectElement() {
  const el = document.getElementById('inputCardToImport')
  el.innerHTML = '<option disabled selected="selected">None selected</option>'
  for (var i = 0; i < localStorageCardKeyList.length; i++) {
    el.innerHTML += '<option>' + localStorageCardKeyList[i] + '</option>'
  }
}

// ── boot: load the card list from the API ────────────────────────────────────

async function initCardStorage() {
  try {
    const res = await fetch('/api/cards')
    if (res.ok) {
      localStorageCardKeyList = await res.json()
      rebuildSelectElement()
    }
  } catch (e) {
    console.error('Could not reach card storage API', e)
  }
}
...
async function saveCard() {
  ...
  if (savedCardKey === null) return   // user cancelled the prompt
  ...
  const res = await fetch('/api/cards/' + encodeURIComponent(savedCardKey), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(cardToBeSaved)
  })
  if (!res.ok) {
    alert('Error saving card: ' + res.statusText)
    return
  }
...
async function deleteCard() {
  if (confirm('Are you sure you want to delete ' + selectedCardKey + '?')) {
    const res = await fetch('/api/cards/' + encodeURIComponent(selectedCardKey), { method: 'DELETE' })
    if (!res.ok) {
      alert('Error deleting card: ' + res.statusText)
      return
    }
...
async function importSavedCard(cardKey = document.getElementById('inputCardToImport').value) {
  if (!cardKey) return
  selectedCardKey = cardKey

  const res = await fetch('/api/cards/' + encodeURIComponent(cardKey))
  if (!res.ok) {
    alert('Card not found: ' + cardKey)
    return
  }
  var importedCard = await res.json()
...
class savedCard {
  ...
  // save() is handled by saveCard() via the API
}

async function downloadSavedCards() {
  var savedCardsText = ''
  for (var i = 0; i < localStorageCardKeyList.length; i++) {
    const res = await fetch('/api/cards/' + encodeURIComponent(localStorageCardKeyList[i]))
    if (res.ok) {
      const cardData = await res.json()
      savedCardsText += localStorageCardKeyList[i] + '<<cardData>>' + JSON.stringify(cardData) + '<<newCard>>'
    }
  }
...
async function uploadSavedCards(event) {
  ...
  reader.onload = async function () {
    ...
    for (var j = 0; j < uploadedCardList.length; j++) {
      ...
      await fetch('/api/cards/' + encodeURIComponent(savedCardKey), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: uploadedCard[1]   // already a JSON string
      })
